...
#define ETSB_LEFT_BOTICON_X	(SCOREBOARD_X+0)
#define ETSB_LEFT_HEAD_X		(SCOREBOARD_X+32)
...
#define ETSB_RATING_WIDTH	    0 // (6 * BIGCHAR_WIDTH)
#define ETSB_NAME_X			(ETSB_SCORELINE_X)
#define ETSB_SCORE_X			(ETSB_SCORELINE_X + .55 * ETSB_SCORELINE_WIDTH)
#define ETSB_PING_X			(ETSB_SCORELINE_X + .70 * ETSB_SCORELINE_WIDTH)
#define ETSB_TIME_X			(ETSB_SCORELINE_X + .85 * ETSB_SCORELINE_WIDTH)
...
static void CG_DrawClientScoreEternal(int y, score_t* score, float* color, float fade, qboolean largeFormat)
{
	//vec3_t	headAngles;
	clientInfo_t* ci;
	float		iconx;
	...
	if (playerDisconnected)
	{
		...
		hcolor[3] = fade * 0.7;
		CG_FillRect(ETSB_SCORELINE_X - 5, y + 2, ETSB_SCORELINE_WIDTH + 10, largeFormat ? SB_NORMAL_HEIGHT : SB_INTER_HEIGHT, hcolor);
	}
	...
	if (!cg_drawScoreboardIcons.integer) {
		CG_Text_Paint(ETSB_NAME_X, y, 0.9f * scale, colorWhite, ci->name, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
	}
	else {
		if (largeFormat) {
			CG_DrawPic(ETSB_NAME_X - 5, y + 2, 25.0f*cgs.widthRatioCoef, 25, ci->modelIcon);
			CG_Text_Paint(ETSB_NAME_X + 24, y, 0.9f * scale, colorWhite, ci->name, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		}
		else {
			CG_DrawPic(ETSB_NAME_X - 5, y + 2, 15 * cgs.widthRatioCoef, 15, ci->modelIcon);
			CG_Text_Paint(ETSB_NAME_X + 12, y, 0.9f * scale, colorWhite, ci->name, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		}
	}
	...
	if (score->ping != -1)
	{
		if (ci->team != TEAM_SPECTATOR || cgs.gametype == GT_TOURNAMENT || playerDisconnected)
		{
			if (cgs.gametype == GT_TOURNAMENT)
			{
				CG_Text_Paint(ETSB_SCORE_X, y, 1.0f * scale, colorWhite, va("%i/%i", ci->wins, ci->losses), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
			}
			else if (cgs.gametype == GT_CTF)
			{
				CG_Text_Paint(ETSB_SCORELINE_X + 0.47f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, va("%i", score->score), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
				CG_Text_Paint(ETSB_SCORELINE_X + 0.59f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, cg_colorScoreboard.integer ? colorYellow : colorWhite, va("%i", score->captures), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
				CG_Text_Paint(ETSB_SCORELINE_X + 0.66f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, cg_colorScoreboard.integer ? colorCyan : colorWhite, va("%i", score->impressiveCount), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //i think this is ret frags? but idk
				CG_Text_Paint(ETSB_SCORELINE_X + 0.72f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, cg_colorScoreboard.integer ? colorMagenta : colorWhite, va("%i", score->defendCount), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);//loda
			}
			else
			{
				CG_Text_Paint(ETSB_SCORE_X, y, 1.0f * scale, colorWhite, va("%i", score->score), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
			}
		}
		...
		{
			if (ci->botSkill != 0)
				CG_Text_Paint(ETSB_SCORELINE_X + 0.80 * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, cg_colorScoreboard.integer ? colorGreen : colorWhite, "BOT", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
			else
				CG_Text_Paint(ETSB_SCORELINE_X + 0.80 * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, cg_colorScoreboard.integer ? colorGreen : colorWhite, va("%i", score->ping), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);

			CG_Text_Paint(ETSB_SCORELINE_X + 0.90 * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, va("%i", score->time), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
		}
		else
		{
			if (ci->botSkill != 0)
				CG_Text_Paint(ETSB_PING_X, y, 1.0f * scale, colorWhite, "BOT", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
			else
				CG_Text_Paint(ETSB_PING_X, y, 1.0f * scale, colorWhite, va("%i", score->ping), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);

			CG_Text_Paint(ETSB_TIME_X, y, 1.0f * scale, colorWhite, va("%i", score->time), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
		}
	}
	else if (cgs.gametype == GT_CTF)
	{
		CG_Text_Paint(ETSB_SCORELINE_X + 0.47f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //score
		CG_Text_Paint(ETSB_SCORELINE_X + 0.59f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //caps
		CG_Text_Paint(ETSB_SCORELINE_X + 0.66f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //assists
		CG_Text_Paint(ETSB_SCORELINE_X + 0.73f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //defends
		CG_Text_Paint(ETSB_SCORELINE_X + 0.80f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //ping
		CG_Text_Paint(ETSB_SCORELINE_X + 0.90f * ETSB_SCORELINE_WIDTH, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL); //time
	}
	else
	{
		CG_Text_Paint(ETSB_SCORE_X, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
		CG_Text_Paint(ETSB_PING_X, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
		CG_Text_Paint(ETSB_TIME_X, y, 1.0f * scale, colorWhite, "-", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
	}

	// add the "ready" marker for intermission exiting
	if (cg.snap->ps.stats[STAT_CLIENTS_READY] & (1 << score->client))
	{
		CG_Text_Paint(ETSB_NAME_X - 64, y + 2, 0.7f * scale, colorWhite, CG_GetStripEdString("INGAMETEXT", "READY"), 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
	}
}
...
static int CG_TeamScoreboardEternal(int y, team_t team, float fade, int maxClients, int lineHeight, qboolean countOnly)
{
	int		i;
	score_t* score;
	float	color[4];
	...
	for (i = 0; i < cg.numScores && count < maxClients; i++) {
		...
		if (!countOnly)
		{
			CG_DrawClientScoreEternal(y + lineHeight * count, score, color, fade, lineHeight == SB_NORMAL_HEIGHT);
		}
...
int CG_GetTeamCountEternal(team_t team, int maxClients)
{
	int i = 0;
	int count = 0;
	int playersCountedBitmask = 0;
...
qboolean CG_DrawOldScoreboard( void ) {
	...
	if ( cgs.gametype == GT_CTF ) {
		...
		n1 = CG_TeamScoreboardEternal( y, TEAM_RED, fade, team1MaxCl, lineHeight, qtrue );
		...
		CG_TeamScoreboardEternal( y, TEAM_RED, fade, team1MaxCl, lineHeight, qfalse );
		hcolor[0] = 0.66f;
		hcolor[1] = 0.66f;
		hcolor[2] = 0.66f;
		hcolor[3] = 1.0f;
		...
		n1 = CG_TeamScoreboardEternal( y, TEAM_SPECTATOR, fade, maxClients, lineHeight, qfalse );
		...
		n2 = CG_TeamScoreboardEternal( y, TEAM_BLUE, fade, team2MaxCl, lineHeight, qtrue );
		...
		CG_TeamScoreboardEternal( y, TEAM_BLUE, fade, team2MaxCl, lineHeight, qfalse );
		hcolor[0] = 0.66f;
		hcolor[1] = 0.66f;
		hcolor[2] = 0.66f;
		hcolor[3] = 1.0f;
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		if ( cg.teamScores[0] >= cg.teamScores[1] ) {
			...
			n1 = CG_TeamScoreboardEternal( y, TEAM_BLUE, fade, team1MaxCl, lineHeight, qtrue );
			...
			CG_TeamScoreboardEternal( y, TEAM_BLUE, fade, team1MaxCl, lineHeight, qfalse );
			y += (n1 * lineHeight) + BIGCHAR_HEIGHT;

			//maxClients -= n1;

			n2 = CG_TeamScoreboardEternal( y, TEAM_RED, fade, team2MaxCl, lineHeight, qtrue );
			...
			CG_TeamScoreboardEternal( y, TEAM_RED, fade, team2MaxCl, lineHeight, qfalse );
			y += (n2 * lineHeight) + BIGCHAR_HEIGHT;
		...
		//
		n1 = CG_TeamScoreboardEternal( y, TEAM_FREE, fade, maxClients, lineHeight, qfalse );
		y += (n1 * lineHeight) + BIGCHAR_HEIGHT;
		n2 = CG_TeamScoreboardEternal( y, TEAM_SPECTATOR, fade, maxClients - n1, lineHeight, qfalse );
		y += (n2 * lineHeight) + BIGCHAR_HEIGHT;
	}
...
qboolean CG_DrawOldScoreboardEternal(void) {
		...
		cg.predictedPlayerState.pm_type == PM_INTERMISSION)
	{
		...
		CG_DrawBigString( x, y, s, fade );
		*/
		x = 0.5f * (SCREEN_WIDTH - CG_Text_Width(s, 1.0f, FONT_MEDIUM));
		y = 40;
	...
	//	the localised strings don't fit - since no-one's ever seen them to notice this.  Smegging brilliant. Thanks people.
	//
	CG_Text_Paint(ETSB_NAME_X, y, 1.0f, colorWhite, /*CG_GetStripEdString("MENUS3", "NAME")*/"Name", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
	if (cgs.gametype == GT_TOURNAMENT)
	{
		char sWL[100];
		trap_SP_GetStringTextString("INGAMETEXT_W_L", sWL, sizeof(sWL));

		CG_Text_Paint(ETSB_SCORE_X, y, 1.0f, colorWhite, sWL, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
	}
	else if (cgs.gametype == GT_CTF)
	{
		CG_Text_Paint(ETSB_SCORELINE_X + 0.47f * ETSB_SCORELINE_WIDTH, y, 1.0f, colorWhite, "Score", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		CG_Text_Paint(ETSB_SCORELINE_X + 0.59f * ETSB_SCORELINE_WIDTH, y, 1.0f, cg_colorScoreboard.integer ? colorYellow : colorWhite, "C", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		//CG_Text_Paint ( ETSB_SCORELINE_X + 0.66f * ETSB_SCORELINE_WIDTH, y, 1.0f, colorWhite, "A", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );//loda
		//CG_Text_Paint ( ETSB_SCORELINE_X + 0.73f * ETSB_SCORELINE_WIDTH, y, 1.0f, colorWhite, "D", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );
		CG_Text_Paint(ETSB_SCORELINE_X + 0.66f * ETSB_SCORELINE_WIDTH, y, 1.0f, cg_colorScoreboard.integer ? colorCyan : colorWhite, "R", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);//loda
		CG_Text_Paint(ETSB_SCORELINE_X + 0.72f * ETSB_SCORELINE_WIDTH, y, 1.0f, cg_colorScoreboard.integer ? colorMagenta : colorWhite, "BC", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		CG_Text_Paint(ETSB_SCORELINE_X + 0.80 * ETSB_SCORELINE_WIDTH, y, 1.0f, cg_colorScoreboard.integer ? colorGreen : colorWhite, "Ping", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		CG_Text_Paint(ETSB_SCORELINE_X + 0.90 * ETSB_SCORELINE_WIDTH, y, 1.0f, colorWhite, "Time", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
	}
	else
	{
		CG_Text_Paint(ETSB_SCORE_X, y, 1.0f, colorWhite, /*CG_GetStripEdString("MENUS3", "SCORE")*/"Score", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		CG_Text_Paint(ETSB_PING_X, y, 1.0f, colorWhite, /*CG_GetStripEdString("MENUS0", "PING")*/"Ping", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
		CG_Text_Paint(ETSB_TIME_X, y, 1.0f, colorWhite, /*CG_GetStripEdString("MENUS3", "TIME")*/"Time", 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
	}

	y = SB_TOP;

	// If there are more than ETSB_MAXCLIENTS_NORMAL, use the interleaved scores
	...
	{
		int team1MaxCl = CG_GetTeamCountEternal(TEAM_RED, maxClients);
		int team2MaxCl = CG_GetTeamCountEternal(TEAM_BLUE, maxClients);
		int team3MaxCl = CG_GetTeamCountEternal(TEAM_FREE, maxClients);
		//
		// teamplay scoreboard
		//
		y += lineHeight / 2;
		...
		n1 = CG_TeamScoreboardEternal(y, TEAM_RED, fade, team1MaxCl, lineHeight, qtrue);
		CG_DrawTeamBackground(ETSB_SCORELINE_X - 5, y - topBorderSize, ETSB_SCORELINE_WIDTH + 10, n1 * lineHeight + bottomBorderSize, 0.33f, TEAM_RED);
		CG_TeamScoreboardEternal(y, TEAM_RED, fade, team1MaxCl, lineHeight, qfalse);
		y += (n1 * lineHeight) + BIGCHAR_HEIGHT;
		...
		n2 = CG_TeamScoreboardEternal(y, TEAM_BLUE, fade, team2MaxCl, lineHeight, qtrue);
		CG_DrawTeamBackground(ETSB_SCORELINE_X - 5, y - topBorderSize, ETSB_SCORELINE_WIDTH + 10, n2 * lineHeight + bottomBorderSize, 0.33f, TEAM_BLUE);
		CG_TeamScoreboardEternal(y, TEAM_BLUE, fade, team2MaxCl, lineHeight, qfalse);
		y += (n2 * lineHeight) + BIGCHAR_HEIGHT;

		//maxClients -= n2;

		n3 = CG_TeamScoreboardEternal(y, TEAM_FREE, fade, team3MaxCl, lineHeight, qtrue);
		CG_DrawTeamBackground(ETSB_SCORELINE_X - 5, y - topBorderSize, ETSB_SCORELINE_WIDTH + 10, n3 * lineHeight + bottomBorderSize, 0.33f, TEAM_FREE);
		CG_TeamScoreboardEternal(y, TEAM_FREE, fade, team3MaxCl, lineHeight, qfalse);
		...
		if (cg.teamScores[0] >= cg.teamScores[1]) {
			...
			int team1MaxCl = CG_GetTeamCountEternal(TEAM_BLUE, maxClients);
			int team2MaxCl = CG_GetTeamCountEternal(TEAM_RED, maxClients);
			...
			n1 = CG_TeamScoreboardEternal(y, TEAM_BLUE, fade, team1MaxCl, lineHeight, qtrue);
			CG_DrawTeamBackground(ETSB_SCORELINE_X - 5, y - topBorderSize, ETSB_SCORELINE_WIDTH + 10, n1 * lineHeight + bottomBorderSize, 0.33f, TEAM_BLUE);
			CG_TeamScoreboardEternal(y, TEAM_BLUE, fade, team1MaxCl, lineHeight, qfalse);
			y += (n1 * lineHeight) + BIGCHAR_HEIGHT;
			...
			n2 = CG_TeamScoreboardEternal(y, TEAM_RED, fade, team2MaxCl, lineHeight, qtrue);
			CG_DrawTeamBackground(ETSB_SCORELINE_X - 5, y - topBorderSize, ETSB_SCORELINE_WIDTH + 10, n2 * lineHeight + bottomBorderSize, 0.33f, TEAM_RED);
			CG_TeamScoreboardEternal(y, TEAM_RED, fade, team2MaxCl, lineHeight, qfalse);
			y += (n2 * lineHeight) + BIGCHAR_HEIGHT;
	...
	if (!localClient) {
		...
		for (i = 0; i < cg.numScores; i++) {
			if (cg.scores[i].client == cg.snap->ps.clientNum) {
				CG_DrawClientScoreEternal(y, &cg.scores[i], fadeColor, fade, lineHeight == SB_NORMAL_HEIGHT);
				break;
			}
		}
	}
